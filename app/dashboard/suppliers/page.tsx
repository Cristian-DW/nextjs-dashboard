  ...
  return (
    <div className="w-full">
      <div className="flex items-center justify-between mb-6">
        <h1 className={`${playfair.className} text-2xl md:text-4xl font-bold text-slate-900`}>{t.suppliers.title}</h1>
        <div className="flex gap-2">
          <Link href="/dashboard/orders" className="flex items-center gap-2 h-10 px-4 rounded-xl border border-slate-200 text-sm font-semibold text-slate-600 hover:bg-slate-50 transition-colors">
            <TruckIcon className="w-4 h-4" /> {t.suppliers.purchaseOrders}
          </Link>
      ...
      {suppliers.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm py-20 text-center">
          ...
          <p className="text-slate-400">{t.suppliers.noSuppliers}</p>
          <Link href="/dashboard/suppliers/create" className="mt-3 inline-block text-sm text-indigo-500 hover:underline">{t.suppliers.addFirst}</Link>
        </div>
      ) : (
